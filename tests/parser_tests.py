...
class RedisParserTestCase(unittest.TestCase):
    ...
    def test_string_key_with_compression(self):
        ...
        self.assertEqual(r.databases[0][key.encode('ascii')], value)
    ...
    def test_ziplist_that_compresses_easily(self):
        ...
        for idx, length in enumerate([6, 12, 18, 24, 30, 36]) :
            key = ("".join("a" for x in range(length))).encode('ascii')
            self.assertEqual(key, r.databases[0][b"ziplist_compresses_easily"][idx])
    ...
    def test_regular_set(self):
        ...
        for member in ("alpha", "beta", "gamma", "delta", "phi", "kappa") :
            self.assertIn(member.encode('ascii'), r.databases[0][b"regular_set"], msg=('%s missing' % member))
